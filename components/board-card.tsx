...
import { PenTool, MoreHorizontal, Archive, Trash2, ExternalLink } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function BoardCard({ board, className }: BoardCardProps) {
  ...
  const { projects, archiveBoard, deleteBoard } = useToolingTrackerStore()
  ...
  return (
    <>
      <Card
        ...
        className={cn(
          "p-0 bg-card border-border cursor-pointer group transition-all hover:shadow-md overflow-hidden flex flex-col h-full",
          className
        )}
      ...
      >
        ...
        <div className="relative aspect-video bg-accent/10 flex items-center justify-center border-b border-border overflow-hidden">
          ...
          {/* Toolbar on hover */}
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity p-2">
            <DropdownMenu>
        ...
        <div className="p-3 flex-1 flex flex-col gap-2">
          ...
          {/* Project Badge or "No project" text */}
          ...
          {/* Updated time */}
          <p className="text-xs text-muted-foreground">Updated {relativeTime}</p>
        </div>
      </Card>
